...
/**
 * Creates a base64 thumbnail of given image blob, resizing to fit within
 * `maxWidth` x `maxWidth` box while maintaining aspect ratio.
 *
 * @param {Blob} originalBlob - The original image blob
 * @param {Number} [maxWidth=100] - The maximum width of the thumbnail
	...
	return new Promise((resolve, reject) => {
		img.onload = () => {
			...
			if (ctx) {
				...
				// Convert to base64
				const base64String = canvas.toDataURL('image/png').replace('data:image/png;base64,', ''); // Remove the prefix
